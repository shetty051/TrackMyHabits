...
function HomeContent() {
  ...
  const [rooneyMode, setRooneyMode] = useState<RooneyMode>('idle');
  ...
  if (status === 'authenticated' && session) {
    return (
      <DashboardShell
        ...
        showIntroOnLoad={triggerIntro || searchParams.get('intro') === 'true'}
        onIntroComplete={() => {
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          >
            <div>
              ...
              <h2 style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text)' }}>
                Hello, {session.user?.name || 'Habit Master'} 👋
              </h2>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              ...
              >
                {rooneyMode === 'idle' ? <Maximize2 size={15} /> : <Minimize2 size={15} />}
                {rooneyMode === 'idle' ? 'Prominent' : 'Idle'}
              </button>
              ...
              >
                <Sparkles size={15} /> Play Rooney Intro
              </button>
            </div>
          </div>
          ...
          >
            ...
            />
            <StatBox
              title="Active Habits Due"
              value={`${totalDueTodayCount} Habits`}
              sub={`Scheduled for ${todayObj.toLocaleDateString('en-US', { weekday: 'short' })}`}
