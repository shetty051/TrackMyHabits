...
const GOAL_HABITS: HabitItem[] = [
  ...
  { id: 'walk', title: 'Walk 10,000 Steps Daily', emoji: '👟', color: '#34D399', frequencyType: 'daily', category: 'Routine' },
  { id: 'exercise', title: 'Exercise 3x / Week', emoji: '🏋️‍♂️', color: '#F87171', frequencyType: 'specific-days', category: 'Fitness' },
  ...
  { id: 'learn', title: 'Learn Something New Daily', emoji: '🧠', color: '#3B82F6', frequencyType: 'daily', category: 'Wellness' },
];
...
export default function OnboardingPage() {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const habitsToCreate = GOAL_HABITS.filter((h) => selectedHabitIds[h.id]);
  ...
  const selectedHabitsCount = Object.values(selectedHabitIds).filter(Boolean).length;
            ...
            >
              ...
              {currentStep === 1 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  ...
                  <p style={cardDescStyle}>
                    This helps our AI coach tune habit recommendations to your age and background.
                  </p>
              ...
              {currentStep === 2 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                    {GOAL_HABITS.slice(0, 3).map((habit) => (
                      <HabitToggleRow
                        key={habit.id}
                        habit={habit}
                        selected={!!selectedHabitIds[habit.id]}
              ...
              {currentStep === 3 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <p style={cardDescStyle}>Would you like to track any of these active habits?</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                    {GOAL_HABITS.slice(3, 6).map((habit) => (
                      <HabitToggleRow
                        key={habit.id}
                        habit={habit}
                        selected={!!selectedHabitIds[habit.id]}
              ...
              {currentStep === 4 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <h2 style={cardTitleStyle}>Evening & mindfulness goals</h2>
                  <p style={cardDescStyle}>Select any additional habits to add to your daily dashboard.</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                    {GOAL_HABITS.slice(6, 9).map((habit) => (
                      <HabitToggleRow
                        key={habit.id}
                        habit={habit}
                        selected={!!selectedHabitIds[habit.id]}
              ...
              {currentStep === 5 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  ...
                  <p style={cardDescStyle}>
                    We will update your profile and pre-create <strong>{selectedHabitsCount} active habits</strong> with 3 streak freezes each.
                  </p>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                      <strong>Age & Sex:</strong> {age ? `${age} years` : 'Not specified'} • {sex}
                    </div>
                    <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                      <strong>Pre-created Habits ({selectedHabitsCount}):</strong>
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                      {GOAL_HABITS.filter((h) => selectedHabitIds[h.id]).map((h) => (
                        <span
                          key={h.id}
                          style={{
